...
void Bucket::add_feature(const cv::Point2f point, const int age,
    ...
    if (age < AGE_THRESHOLD) {
        ...
        if (size() < max_size) {
            ...
            int score_min =
                compute_score(features.ages[0], features.strengths[0]);
            int score_min_idx = 0;
            for (int i = 1; i < size(); i++) {
                const int current_score =
                    compute_score(features.ages[i], features.strengths[i]);
...
std::vector<cv::Point2f> visual_odometry::featureDetectionFast(
...
void FeatureSet::appendFeaturesFromImage(const cv::Mat& image,
    ...
    std::vector<cv::Point2f> points_new =
        featureDetectionFast(image, fast_threshold, response_strength);

    points.insert(points.end(), points_new.begin(), points_new.end());
    std::vector<int> ages_new(points_new.size(), 0);
    ages.insert(ages.end(), ages_new.begin(), ages_new.end());
    strengths.insert(strengths.end(), response_strength.begin(),
                     response_strength.end());
...
void FeatureSet::filterByBucketLocationInternal(const cv::Mat& image,
    ...
    for (int buckets_idx_height = 0; buckets_idx_height < buckets_along_height;
         buckets_idx_height++) {
        for (int buckets_idx_width = 0; buckets_idx_width < buckets_along_width;
             buckets_idx_width++) {
            // Ignore top rows of image.
            if (buckets_idx_height >= bucket_start_row)
    ...
    for (int buckets_idx_height = 0; buckets_idx_height < buckets_along_height;
        ...
        for (int buckets_idx_width = 0; buckets_idx_width < buckets_along_width;
             buckets_idx_width++) {
            buckets_idx =
                buckets_idx_height * buckets_along_width + buckets_idx_width;
            FeatureSet bucket_features = buckets[buckets_idx].features;
            points.insert(points.end(), bucket_features.points.begin(),
                          bucket_features.points.end());
            ages.insert(ages.end(), bucket_features.ages.begin(),
                        bucket_features.ages.end());
            strengths.insert(strengths.end(), bucket_features.strengths.begin(),
                             bucket_features.strengths.end());
        }
    }
}
void FeatureSet::filterByBucketLocation(const cv::Mat& image) {
    filterByBucketLocationInternal(image, BUCKETS_ALONG_HEIGHT,
                                   BUCKETS_ALONG_WIDTH, BUCKET_START_ROW,
                                   FEATURES_PER_BUCKET);
}
